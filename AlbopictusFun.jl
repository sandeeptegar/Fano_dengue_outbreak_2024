# function dengue ()
function dengue(latitude, longitude, t_start::String, t_end::String, seed_date::String)
    ...
    function infect(time)
        ...
        # A person remain infectious from 2 t0 7 days (WHO), therefore, 
        # we keep single seeding active upto 7 days including the symptom onset day. 
    ...
    function aedes_model(dy,y,h,p,t)
        ...
        R_E_D_lag = (1 - delta_E_D(TEMP_NOW)) * H(t) * y[numb_E_D] ;
        R_E_Q     = Qui(t) .* (R_E_lag .+ R_E_D_lag)
        R_E_Q_mat = (1 - delta_E_Q(TEMP_NOW)) * R(t) * y[numb_E_Q]
        R_L     = (1-Qui(t )) * Dia_E .* (g_E(TEMP_NOW) /  g_E(TEMP_E) ) * sum(repeat(q_func(TEMP_E),2) .* ylag_E[numb_A_1:numb_I_2] ./ (G_Time(TEMP_E))) .* y[numb_P_E]  .+
        (1-Qui(t )) * (1 - delta_E_D(TEMP_NOW)) * H(t) * y[numb_E_D] .+
        (1 - delta_E_Q(TEMP_NOW)) * R(t) * y[numb_E_Q] .+
        (g_E(TEMP_NOW) /  g_E(TEMP_E) ) * on_off(t-y[numb_t_E],0)[1] .* y[numb_P_E]
        ...
        (g_L(TEMP_NOW, v ) ./ g_L(TEMP_L, v_L )) * (1 - delta_E_D(TEMP_L)) * R(t - y[numb_t_L]) * ylag_L[numb_E_Q] .* y[numb_P_L] .+
        (g_E(TEMP_L ) / g_E(TEMP_E_L)) * (g_L(TEMP_NOW, v ) ./ g_L(TEMP_L, v_L )) * (1 - delta_E_Q(TEMP_L)) * on_off(t-ylag_L[numb_t_E] - y[numb_t_L],0)[1].*  ylag_L[numb_P_E]  * y[numb_P_L]
        ...
        (1-Qui(t  - ylag_P[numb_t_L] - y[numb_t_P]))*w_A .*(g_L(TEMP_P, v_P) ./ g_L(TEMP_L_P, v_L_P)) * (g_P(TEMP_NOW) ./ g_P(TEMP_P)) .*(1 - delta_E_D(TEMP_L_P)) .*H(t - ylag_P[numb_t_L] - y[numb_t_P]) .* ylag_L_P[numb_E_D] .* ylag_P[numb_P_L] .* y[numb_P_P] .+
        w_A .*(g_L(TEMP_P, v_P) ./ g_L(TEMP_L_P, v_L_P)) * (g_P(TEMP_NOW) ./ g_P(TEMP_P)) .*  (1 - delta_E_Q(TEMP_L_P)) .*R(t - ylag_P[numb_t_L] - y[numb_t_P]) .* ylag_L_P[numb_E_Q].* ylag_P[numb_P_L] .* y[numb_P_P] .+
        ...
        dy[numb_H_I]                  = .+ vector_human(Temp(t - 4)) .* bite(Temp(t - 4))   .*  40* 4  .* abs(sum(ylag_inf[numb_I_1:numb_I_2])) .* ylag_inf[numb_H_S] ./ (ylag_inf[numb_H_S] + ylag_inf[numb_H_I] + ylag_inf[numb_H_R]  .+ 15000     ) .+ infect(t) .- infect(t-4)  .-  (vector_human(Temp(t - 8)) .* bite(Temp(t - 8))) .*   40* 4  .* sum( ylag_rev[numb_I_1:numb_I_2]) .* ylag_rev[numb_H_S] ./ (ylag_rev[numb_H_S] + ylag_rev[numb_H_I] + ylag_rev[numb_H_R]  .+ 15000     )
